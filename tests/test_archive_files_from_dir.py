...
from tests.conftest import resourse_path
...
def test_file_list(open_archive):
    with open_archive as zf:
        assert zf.namelist() == os.listdir(resourse_path)


def test_csv_file(open_archive):
    with open_archive as zf:
        name = 'username.csv'
        assert zf.getinfo(name).file_size == os.path.getsize(os.path.join(resourse_path, name))
        z_file = zf.read(name)
        with open(os.path.join(resourse_path, name), 'rb') as file:
            csv_file = file.read()
...
def test_pdf_file(open_archive):
    with open_archive as zf:
        ...
        reader = PdfReader(os.path.join(resourse_path, name))
...
def test_xls_file(open_archive):
    with open_archive as zf:
        ...
        book = open_workbook(os.path.join(resourse_path, name))
